...
public class LargeSpikeFeature extends Feature<LargeDripstoneFeatureConfig> {
    public LargeSpikeFeature(Codec<LargeDripstoneFeatureConfig> codec) {
        super(codec);
    }
    ...
    private void testGeneration(StructureWorldAccess world, BlockPos pos, CaveSurface.Bounded surface, WindModifier wind) {
        ...
        while (mutable.getY() < surface.getCeiling() - 1) {
            ...
            if (SpikeHelper.canGenerate(world, blockPos) || world.getBlockState(blockPos).isOf(Blocks.BLACKSTONE)) {
    ...
    static final class DripstoneGenerator {
        ...
        boolean canGenerate(StructureWorldAccess world, WindModifier wind) {
            while (this.scale > 1) {
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    if (SpikeHelper.canGenerateBase(world, wind.modify(mutable), this.scale)) {
                        this.pos = mutable;
                        return true;
                    }
        ...
        private int scale(float height) {
            return (int)SpikeHelper.scaleHeightFromRadius(height, this.scale, this.heightScale, this.bluntness);
        }
            ...
            for (int i = -this.scale; i <= this.scale; ++i) {
                block1: for (int j = -this.scale; j <= this.scale; ++j) {
                    ...
                    for (int m = 0; m < k && mutable.getY() < l; ++m) {
                        BlockPos blockPos = wind.modify(mutable);
                        if (SpikeHelper.canGenerateOrLava(world, blockPos)) {
                            bl = true;
