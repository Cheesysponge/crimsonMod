...
public class SpikeClusterFeature extends Feature<DripstoneClusterFeatureConfig> {
    public SpikeClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<DripstoneClusterFeatureConfig> context) {
        ...
        if (!SpikeHelper.canGenerate(structureWorldAccess, blockPos)) {
            return false;
        }
    ...
    private void generate(StructureWorldAccess world, Random random, BlockPos pos, int localX, int localZ, float wetness, double dripstoneChance, int height, float density, DripstoneClusterFeatureConfig config) {
        ...
        boolean bl;
        Optional<CaveSurface> optional = CaveSurface.create(world, pos, config.floorToCeilingSearchRange, SpikeHelper::canGenerate, SpikeHelper::cannotGenerate);
        ...
        if (optionalInt.isPresent()) {
            SpikeHelper.generatePointedDripstone(world, pos.withY(optionalInt.getAsInt() - 1), Direction.DOWN, m, bl4);
        }
        if (optionalInt3.isPresent()) {
            SpikeHelper.generatePointedDripstone(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
        }
    }
    ...
    private void placeDripstoneBlocks(StructureWorldAccess world, BlockPos pos, int height, Direction direction) {
        ...
        for (int i = 0; i < height; ++i) {
            if (!SpikeHelper.generateDripstoneBlock(world, mutable)) {
                return;
            }
